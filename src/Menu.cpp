...
#endif
#include "HitoriLevels.h"

int MainMenu::s_nLevel = 0;

MainMenu::MainMenu(SDL_Surface* pScreen, Config* pConfig)
: m_pScreen(pScreen), m_eChoice(Play), m_pConfig(pConfig), m_bBeatLevel(false)
#ifdef SHOW_LEVEL_DIMENSIONS
, m_nWidth(-1), m_nHeight(-1)
#endif
{
	m_pBackground = nSDL_LoadImage(image_HitoriMainMenu);
	m_pStar = nSDL_LoadImage(image_HitoriStar);
...
void MainMenu::UpdateDisplay()
{
	...
	if( is_classic ) {
        ...
        Hit_itoa(s_nLevel+1, buffer + 6, 16-6);
	...
	if( m_nWidth > 0 && m_nHeight > 0 ) {
		...
		char buf[8];
		strcpy(bufMessage, "(");
		Hit_itoa(m_nWidth, buf, 8);
		strcat(bufMessage, buf);
		strcat(bufMessage, ", ");
		Hit_itoa(m_nHeight, buf, 8);
		strcat(bufMessage, buf);
		strcat(bufMessage, ")");
...
{
	m_bBeatLevel = m_pConfig->BeatLevel(s_nLevel);
#ifdef SHOW_LEVEL_DIMENSIONS
	HitoriLib hit;
